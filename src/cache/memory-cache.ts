...
export class MemoryCache {
  ...
  async set<T>(key: CacheKey, value: T, ttl?: number): Promise<void> {
    ...
    if (dbg.isEnabled()) {
      dbg.log('CACHE', 'memory-cache', `SET: ${key}`, { key, size: entrySize, ttl: effectiveTTL })
    }
  }
  ...
  async get<T>(key: CacheKey): Promise<T | null> {
    ...
    if (!entry || entry.expiresAt <= Date.now()) {
      ...
      const dbg = getDebugLogger()
      if (dbg.isEnabled()) {
        dbg.log('CACHE', 'memory-cache', `MISS: ${key}`, { key })
      }
      return null
    }
    ...
    const dbg = getDebugLogger()
    if (dbg.isEnabled()) {
      dbg.log('CACHE', 'memory-cache', `HIT: ${key}`, { key, size: this.entryByteSize(fullKey, entry.value) })
    }
