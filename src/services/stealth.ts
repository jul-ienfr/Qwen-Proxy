    ...
    (function() {
      ...
      (function() {
        function makeMime(desc, suffixes, type) {
          return { description: desc, suffixes, type };
        }
        ...
        const pdfPlugin = { name: 'PDF Viewer', description: 'Portable Document Format', filename: 'internal-pdf-viewer', length: 2, 0: pdfMime, 1: pdfxMime };
        ...
        const chromePdfPlugin = { name: 'Chrome PDF Viewer', description: 'Portable Document Format', filename: 'internal-pdf-viewer', length: 2, 0: chromePdfMime, 1: chromePdfMime2 };
        chromePdfMime.enabledPlugin = chromePdfPlugin;
        chromePdfMime2.enabledPlugin = chromePdfPlugin;

        const nativePlugin = { name: 'Native Client', description: '', filename: 'internal-nacl-plugin', length: 2, 0: makeMime('Native Client Executable', '', 'application/x-nacl'), 1: makeMime('Portable Native Client Executable', '', 'application/x-pnacl') };
        ...
        defineOnPrototype(navigator, 'plugins', makeNamedNodeMap(pluginsList, pluginsList.map((p, i) => [p.name, p])));
        defineOnPrototype(navigator, 'mimeTypes', makeNamedNodeMap(mimeList, mimeList.map(m => [m.type, m])));
      })();
      ...
      (function() {
        ...
        HTMLCanvasElement.prototype.toDataURL = function(...args) { addNoise(this); return _toDataURL.apply(this, args); };
        spoofedFunctions.add(HTMLCanvasElement.prototype.toDataURL);
        HTMLCanvasElement.prototype.toBlob = function(...args) { addNoise(this); return _toBlob.apply(this, args); };
