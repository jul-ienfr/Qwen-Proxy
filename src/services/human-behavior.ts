...
export async function humanType(page: Page, selector: string, text: string): Promise<void> {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (char === ' ' || char === '.' || char === ',') {
      if (simpleRng() < 0.4) await sleep(logNormalDelay(simpleRng, 150, 500));
    }
    if (simpleRng() < 0.05) await sleep(logNormalDelay(simpleRng, 200, 800));
  }
}

export async function humanMouseMove(
  page: Page, fromX: number, fromY: number, toX: number, toY: number,
  options: { overshoot?: number; steps?: number; pauseAt?: number[] } = {},
): Promise<void> {
  const { overshoot = 5 + Math.floor(simpleRng() * 15), steps = 20 + Math.floor(simpleRng() * 15), pauseAt = [0.3, 0.7] } = options;
  const dx = toX - fromX, dy = toY - fromY, distance = Math.sqrt(dx * dx + dy * dy);
  ...
  for (let i = 1; i <= steps; i++) {
    ...
    if (t < 0.85) { const a = t / 0.85; x = cubicBezier(fromX, cp1x, cp2x, overshootX, a); y = cubicBezier(fromY, cp1y, cp2y, overshootY, a); }
    else { const c = (t - 0.85) / 0.15; x = overshootX + (toX - overshootX) * c; y = overshootY + (toY - overshootY) * c; }
    x += gaussianRandom(simpleRng) * 1.5; y += gaussianRandom(simpleRng) * 1.5;
    await page.mouse.move(x, y, { steps: 2 });
    if (pauseAt.some(p => Math.abs(t - p) < 0.05) && simpleRng() < 0.6) await sleep(logNormalDelay(simpleRng, 15, 50));
...
export async function humanDrag(page: Page, fromX: number, fromY: number, toX: number, toY: number): Promise<void> {
  ...
  const overshoot = 8 + Math.floor(simpleRng() * 12), steps = 30 + Math.floor(simpleRng() * 20);
  ...
  for (let i = 1; i <= steps; i++) {
    ...
    x += gaussianRandom(simpleRng) * 1.2; y += gaussianRandom(simpleRng) * 1.2;
    await page.mouse.move(x, y, { steps: 2 });
    if ((Math.abs(t - 0.3) < 0.03 || Math.abs(t - 0.65) < 0.03) && simpleRng() < 0.5) await sleep(logNormalDelay(simpleRng, 20, 60));
...
export async function humanScroll(page: Page): Promise<void> {
  ...
  if (simpleRng() < 0.3) { await page.mouse.wheel(0, scrollAmount * 0.3 * direction); await sleep(logNormalDelay(simpleRng, 200, 500)); }
}
...
export async function humanPagePresence(page: Page): Promise<void> {
  ...
  for (let i = 0; i < points; i++) {
    const x = Math.floor(simpleRng() * viewport.width), y = Math.floor(simpleRng() * viewport.height);
    await humanMouseMove(page, Math.floor(simpleRng() * viewport.width), Math.floor(simpleRng() * viewport.height), x, y, { overshoot: 0 });
    await sleep(logNormalDelay(simpleRng, 500, 1500));
  }
  if (simpleRng() < 0.5) await humanScroll(page);
