...
export async function getBasicHeaders(accountId?: string): Promise<{ cookie: string, userAgent: string, bxV: string, bxUa?: string, bxUmidtoken?: string }> {
  ...
  let page = (accountId && accountId !== '_default') ? accountPages.get(accountId) : getActivePage();
  if (accountId && accountId !== '_default' && !page) {
  ...
  if (!bxUa || !bxUmidtoken) {
    ...
    if (diskCache && diskCache.bxUa && diskCache.bxUmidtoken) {
      ...
      bxUa = diskCache.bxUa;
      bxUmidtoken = diskCache.bxUmidtoken;
      cache.currentHeaders['bx-ua'] = bxUa;
      cache.currentHeaders['bx-umidtoken'] = bxUmidtoken;
      cache.currentHeaders['bx-v'] = diskCache.bxV;
      cache.currentHeaders['cookie'] = diskCache.cookie;
      return {
        cookie: diskCache.cookie,
        userAgent: diskCache.userAgent,
        bxV: diskCache.bxV,
        bxUa,
        bxUmidtoken,
      };
...
async function _getQwenHeadersInternalOnce(forceNew = false, accountId?: string): Promise<{ headers: Record<string, string>, chatSessionId: string, parentMessageId: string | null }> {
  ...
  if (accountId && accountId !== '_default' && !accountPages.has(accountId)) {
    const { getAccountCredentials } = await import('../core/accounts.js');
    const creds = getAccountCredentials(accountId);
    if (creds) {
      await initPlaywrightForAccount(creds, config.browser.headless);
  ...
  const page = (accountId && accountId !== '_default') ? accountPages.get(accountId) : getActivePage();
  if (!page) {
    ...
    return await new Promise<{ headers: Record<string, string>, chatSessionId: string, parentMessageId: string | null }>((resolve, reject) => {
      ...
      const routeHandler = async (route: any, request: any) => {
        ...
        saveCachedHeaders(cacheKey, {
          ...
          bxUa: extractedHeaders['bx-ua'] || '',
          bxUmidtoken: extractedHeaders['bx-umidtoken'] || '',
        });
